...
import org.ginga.toolbox.util.SpectraMpc3Fixer;

public class SpectraMpc3FixerCmd {

    public static final Logger log = Logger.getLogger(SpectraMpc3FixerCmd.class);

    public void generate(File f) {
        DatabaseSchemaGenerator schemaGen = new DatabaseSchemaGenerator();
        List<Class<?>> annotatedClasses = new ArrayList<>();
        annotatedClasses.add(LacdumpSfEntity.class);
    ...
    public static void main(String[] args) {
        try {
            ...
            if (commandLine.hasOption("d")) {
                String directoryPath = commandLine.getOptionValue("d");
    ...
    private static void printHelp() {
        ...
        helpFormatter.printHelp("fix_mpc3_spectra.sh", getOptions());
    }

    @SuppressWarnings("static-access")
    private static Options getOptions() {
        Options options = new Options();
        Option directoryOption = OptionBuilder.withArgName("path").withLongOpt("directory")
                .withDescription("directory where the MPC3 pha files are located").hasArg()
                .create("d");
