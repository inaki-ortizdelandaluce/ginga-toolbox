...
public class ObservationBgDatabaseIngestor {
    ...
    public ObservationBgDatabaseIngestor() {

    }

    public void ingest(File dataRootDir) {
        ...
        try {
            ...
            for (ObservationEntity obs : obsList) {
                if (obs.getSequenceNumber() % 10 == 9) { // ends with 9, bgd observation
                    lacdumpDir = new File(dataRootDir, String.valueOf(obs.getSequenceNumber()
                            + File.separator + LACDUMP_DIRECTORY));
                } else { // ends with 0, normal observation
                    lacdumpDir = new File(dataRootDir, String.valueOf(obs.getSequenceNumber()
                            + File.separator + LACDUMP_BGD_DIRECTORY));
                }
                lacdumpFileNames = getFileNames(lacdumpDir
                        .listFiles(new FileUtil.LacdumpFileFilter()));
                log.info(lacdumpFileNames.length
                        + " background LACDUMP file(s) found for observation "
                        + obs.getSequenceNumber());
                ...
                dao.update(obs);
                log.info("Observation " + obs.getSequenceNumber()
            ...
            log.error(
                    "Error searching/updating Ginga observations stored in the database. Message="
                            + e.getMessage(), e);
            System.exit(1);
        }
    }
